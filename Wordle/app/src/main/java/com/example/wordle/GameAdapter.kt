    ...
    fun checkAnswer(answer: String, etList: ArrayList<EditText>) {
        ...
        for (i in 0 until etList.size) {
            ...
            if (answerChar == etChar) {
                etList.get(i).setBackgroundResource(R.drawable.radius_green)
            } else {
                var check = true

                for (j in 0 until etList.size) {
                    if (etChar == answer.get(j)) {
                        etList.get(i).setBackgroundResource(R.drawable.radius_yellow)
                        check = false
                    }
                }

                if (check) {
                    etList.get(i).setBackgroundResource(R.drawable.radius_gray)
                }
            }
        }
    }
